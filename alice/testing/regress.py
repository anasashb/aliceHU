...
def t_test(trues, x, y):
    """
    Wrapper function for scipy.stats implementation of t-test of
    independence. Incorporating the Levene test, function automatically
    picks whether to use t-test or Welch's t-test.

    Args:
        trues (array-like): True values - an unused variable only there
                            to ensure smooth implementation of testing
                            in cases where other other tests require
                            true values
                            (refer to mcnemar tests classifier tests).
